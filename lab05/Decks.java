...
public class Decks
{
   ...
   public Decks(int num)
   {
      ...
      int count = 0;
      for (int i = 0; i < SIZE; i++) //Cycle through the decks
      {
         ...
         //you will need a counter to insert the Cards into the decks array
           ...
           decks[i] = new Card((i % STANDARD_DECK_SIZE) + 1);
   ...
   public void shuffle()
   {
      //use the Permutation class to shuffle the Cards
        ...
        Permutation shuffler = new Permutation(SIZE, SIZE); //Create shuffler; pick SIZE numbers from SIZE numbers
        
        Card[] cardArray = new Card[SIZE]; //Make temp array of Cards
        
        /*for (int i = 1; i <= numDecks; i++){
             
             for (int j = 1; j <= STANDARD_DECK_SIZE; j++){
                  
                  cardArray[(i * j) - 1] = decks[(shuffler.next() - 1)];
        ...
        for (int i = 0; i < SIZE; i++){
             ...
             cardArray[i] = decks[shuffler.next() - 1];
